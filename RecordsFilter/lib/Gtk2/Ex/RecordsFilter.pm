...
B<Gtk2::Ex::RecordsFilter> - A high level widget to browse reasonably large amounts of relational data and  select a subset of records. This widget is inspired by the song browser of iTunes.
...
=head1 SYNOPSIS
   ...
   # Create the recordsfilter object using this recordset   
...
Takes an array of arrays (a set or relational records ) as the input. The widget can be used to select a subset of these records (into a selection list). The methods C<get_selected_rows> and C<get_unselected_rows> can be used to view the selection ( or unselection) at any point.
